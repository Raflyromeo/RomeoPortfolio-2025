...
const handleDownload = () => {
    ...
    const interval = setInterval(() => {
        progress.value += 5; // increment progress
        if (progress.value >= 100) {
            clearInterval(interval);
            downloadStatus.value = 'downloaded';
            
            // Real physical download trigger
    ...
    }, 100); // 100ms * 20 = 2 seconds total animation
};
...
onMounted(() => {
  ...
  const type = () => {
    ...
    if (!isDeleting.value && currentTypingText.value === currentRole) {
      ...
      typingSpeed = 500; // Pause before start typing new word
    }
    
    timerId = setTimeout(type, typingSpeed);
  };
  
  timerId = setTimeout(type, 1500); // Wait for intro animation to finish before typing
  ...
  setTimeout(() => {
    ...
    // Scroll Trigger Anti-gravity float
    ...
    // Animate the entire container during scroll instead of individual elements 
    // to prevent animation conflicts with the entrance timeline
    ...
    <div class="w-[90%] md:w-[85%] max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-12 items-center gap-12 lg:gap-20 z-10 relative">
      ...
      <div class="col-span-1 lg:col-span-5 flex justify-center lg:justify-end relative z-10 mt-10 lg:mt-0">
        <div ref="imageGroup" class="relative group perspective-[1000px]">
          ...
          <!-- Futuristic Glassmorphism Badge -->
